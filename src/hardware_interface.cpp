...
void RobotHardwareInterface::init(){
    ...
    for (int i = 0; i < num_joints_; ++i) {
        ...
        getSoftJointLimits(urdf_joint, soft_limits);

        // Define manually some limits
        ...
        hardware_interface::JointHandle jointPositionHandle = position_joint_interface_.getHandle(joint_names_[i]);
...
void RobotHardwareInterface::write(ros::Duration elapsed_time){
    ...
    //ROS_INFO("Before enforce: %f",joint_position_command_[0] );
    positionJointSoftLimitsInterface.enforceLimits(elapsed_time);
    //ROS_INFO("After enforce: %f",joint_position_command_[0] );
